...
class _AddEmployeeScreenState extends ConsumerState<AddEmployeeScreen> {
  ...
  void _submit() async {
    ...
    if (data['workEmail'] == null || data['workEmail'].toString().isEmpty) {
      ref.read(employeeFormProvider.notifier).setError('Work Email is required (Basic Info)');
      return;
    }
    if (data['phone'] == null || data['phone'].toString().isEmpty) {
      ref.read(employeeFormProvider.notifier).setError('Phone is required (Basic Info)');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ref.read(employeeFormProvider.notifier).setError('Failed to create employee: $e');
      }
    } finally {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_pageController.hasClients && _pageController.page?.round() != formState.currentStep) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text('Add Employee', style: AppTypography.title.copyWith(fontWeight: FontWeight.w700)),
        ...
        children: [
          ...
          if (formState.error != null)
            Container(
              ...
              color: AppColors.destructiveLight.withValues(alpha: isDark ? 0.2 : 0.1),
              child: Row(
                children: [
                  const Icon(LucideIcons.alertCircle, color: AppColors.destructiveLight, size: 20),
                  const SizedBox(width: AppSpacing.s8),
                  Expanded(child: Text(formState.error!, style: const TextStyle(color: AppColors.destructiveLight))),
                  IconButton(
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                OutlinedButton(
                  onPressed: formState.currentStep > 0 ? formNotifier.previousStep : null,
                  child: const Text('Previous'),
                ),
                ElevatedButton(
                  onPressed: formState.isLoading 
                    ? null 
                    : (formState.currentStep < _stepTitles.length - 1 ? formNotifier.nextStep : _submit),
                  style: ElevatedButton.styleFrom(
                    backgroundColor: AppColors.primaryLight,
                    foregroundColor: Colors.white,
                  ),
                  child: formState.isLoading && formState.currentStep == _stepTitles.length - 1
                    ? const SizedBox(width: 16, height: 16, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                    : Text(formState.currentStep < _stepTitles.length - 1 ? 'Save & Next' : 'Submit'),
                ),
              ],
            ),
          ),
